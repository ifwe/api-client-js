...
describe('Tagged API', function() {
    ...
    it('api.execute should not submit prototypal properties', function(){
        var expectedBody = "\nmethod=my.api&defaultParam1=defaultValue1&defaultParam2=defaultValue2&foo=bar&baz=bot\n";
    ...
    it('api.execute makes post with transformed post body and correct endpoint', function() {
        var expectedBody = "\nmethod=im.send&defaultParam1=defaultValue1&defaultParam2=defaultValue2&param1=foo&param2=bar\n";
    ...
    it('api.execute should add parameters to query string', function() {
        ...
        this.clock.tick(1);
        this.http.post.lastCall.args[0].should.have.property('url', this.endpoint + "?foo=123&bar=abc");
    });

    it('api.execute makes post with encoded keys', function() {
        var expectedBody = "\nmethod=im.send&defaultParam1=defaultValue1&defaultParam2=defaultValue2&pb%26j=foo\n";
        this.api.execute("im.send", {
            "pb&j": "foo"
        });
        this.clock.tick(1);
    ...
    it('api.execute makes post with encoded parameter values', function() {
        var expectedBody = "\nmethod=im.send&defaultParam1=defaultValue1&defaultParam2=defaultValue2&param1=foo%26bar\n";
        this.api.execute("im.send", {
            param1: "foo&bar"
        });
        this.clock.tick(1);
